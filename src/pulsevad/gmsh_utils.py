...
def helixWireGenerator(helix_radius, helix_angle, length, r_inner, r_outer, removeTool=False, n_points=200):
    
    helix_angle = np.radians(helix_angle)
    pitch = 2 * np.pi * helix_radius * np.tan(helix_angle)
...
def helixWireGenerator2(helix_radius, pitch, n_turns, r_inner, r_outer, removeTool=False, n_points=200):
